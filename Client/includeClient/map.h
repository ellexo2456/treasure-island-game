...
public:
    ...
    void render(sf::RenderWindow& window) {
        for (int i = 0; i < HEIGHT_MAP; i++) // идём по строкам
            ...
            {
                if (TileMap[i][j] == 's')  hero_sprite.setTextureRect(
                        sf::IntRect(192, 0, 64, 64));//если встретили символ s, то рисуем 2й квадратик
                if ((TileMap[i][j] == '0')) hero_sprite.setTextureRect(
